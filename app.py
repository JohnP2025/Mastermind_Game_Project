...
def new_game():
    ...
    colors = ["Orange", "Yellow", "Green", "Cyan", "Indigo", "Pink"] # will not be hardcoded, see below
    maxGuesses = 10 # colors and maxGuesses are hardcoded for testing, but may add customization to web interface
    
    game_data = mastermind_Logic.new_game(colors, maxGuesses) # store all information resulting from new_game call in dictionary
...
def guess():
    ...
    if not game: # returns an error if game cannot be found in memory
        return {"success": False, "data": None, "error": "Invalid game_id"}, 404
    ...
    if guess_data["status"]==1: # if guess is correct
        ...
        return {"success": False, "data": None, "error": "Invalid guess."}, 400
